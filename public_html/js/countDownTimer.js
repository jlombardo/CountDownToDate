...
ByteShopJs.util.datetime.CountDownTimer = function() {
    ...
    var timeToGo;
    // Cached DOM element references for NONE format (avoids repeated getElementById per tick)
    var daysEl, hoursEl, minsEl, secsEl;
    ...
    var startUsingSecondsToEndDate = function(secondsToEndDate, timeToGoElement, displayFormat) {
        endTimestamp = Date.now() + (secondsToEndDate * 1000);
        timeToGo = timeToGoElement;
        // default if missing
        format = !displayFormat ? "MEDIUM" : displayFormat;
        if (format === "NONE") {
    ...
    var countDown = function() {
        // Anchor remaining time to the real clock to prevent setInterval drift
        var remaining = Math.max(0, Math.ceil((endTimestamp - Date.now()) / 1000));
        ...
        if (format === "NONE") {
            ...
            timeToGo.innerHTML = formatValue(remaining);
        }
    };
    ...
    return {
        // Most properties and functions are private, but we'll expose these.
